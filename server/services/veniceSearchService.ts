...
export async function searchForGrants(query: string, user?: User): Promise<SearchResult[]> {
  ...
  try {
    ...
    const userPreference = user?.preferredLlmModel && user.preferredLlmModel !== 'venice' 
    ...
    const response = await axios.post(
      `${VENICE_API_BASE_URL}/chat/completions`,
      {
        ...
        messages: [
          {
            role: "system",
            content: "You are a helpful assistant that searches the web for all the relevant grants available and returns the web search results to me"
          },
          {
            role: "user",
            content: `Tell me about research grants available for ${query}`
          }
        ],
        venice_parameters: {
    ...
    throw new Error('Failed to search for grants: ' + (error instanceof Error ? error.message : 'Unknown error'));
  }
}
...
function extractGrantsFromAIResponse(aiResponse: string): SearchResult[] {
  ...
  for (let i = 0; i < paragraphs.length; i++) {
    ...
    if (/grant|funding|award|scholarship|fellowship/i.test(paragraph)) {
      ...
      const urlMatch = paragraph.match(/https?:\/\/[^\s()<>]+(?:\([^\s()<>]+\)|([^[:punct:]\s]|\/?))/);
      if (urlMatch) {
        url = urlMatch[0];
      ...
      const titleMatch = paragraph.match(/^(.*?(?:Grant|Fund|Award|Program|Fellowship|Scholarship).*?)(?:\.|:|\n)/i);
      if (titleMatch) {
  ...
  if (grants.length === 0 && aiResponse.length > 0) {
    grants.push({
      ...
      url: '',
      snippet: aiResponse.substring(0, 500) + (aiResponse.length > 500 ? '...' : ''),
...
function extractAmount(text: string): string {
  ...
  const currencyRegex = /[$£€]\s*(\d{1,3}(,\d{3})*(\.\d+)?)\s*(million|m|k|thousand)?/gi;
  const numericRegex = /(\d{1,3}(,\d{3})*(\.\d+)?)\s*(million|m|k|thousand)?\s*(dollars|pounds|euros)?/gi;
...
function determineGrantType(text: string, url: string, title: string): 'government' | 'private' {
  // Government indicators in domain or text
  const govDomains = ['gov', 'government', 'fed', 'federal', 'state', 'council', 'ministry', 'department'];
  const govKeywords = ['federal', 'government', 'national', 'state', 'public funding', 'public grant'];
  ...
  const govOrgs = [
    'national', 'federal', 'ministry', 'department', 'agency', 'council',
    'NSF', 'NIH', 'CDC', 'NASA', 'DOE', 'EPA', 'USDA', 'European Commission',
    'Research Council'
  ];
